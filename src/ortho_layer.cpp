...
#include "opt.h"
using namespace std;
int UNIT_ATOM_NUM_ORTHO_LAYER = 6;

extern int shape(string shape, double x, double y, double z);

int ortho_layer(int argc, char **argv, optpara opts){
        ...
        if(opts.atomcomp == "zl-3HO-Si") {
          atm = "Si";
          lattice_a = 9.40676;
          lattice_b = 6.65159;
          lattice_c = 3.84030;
        }
        else if(opts.atomcomp == "zl1-3HO-Si") {
          atm = "Si";
          one_layer = 1;
          lattice_a = 9.40676;
          lattice_b = 6.65159;
        ...
        Coordinate atom_zl2[6] = {
                {0, 0.5, 0}, {0.25, 0.5, 0}, {0.333333, 0.833333, 0},
                {0.583333, 0.833333, 0}, {0.666667, 0.166667, 0}, {0.916667, 0.166667, 0}
        };
	 ...
	 for(m=0; m<atmlist.size(); m++){
        ...
        for(k=0; k<nz; k++){
          for(j=0; j<ny; j++){
            for(i=0; i<nx; i++){
                for(l=0; l<UNIT_ATOM_NUM_ORTHO_LAYER; l++){
                      double ax, ay, az;
