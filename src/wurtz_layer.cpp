...
#include "opt.h"
using namespace std;
int UNIT_ATOM_NUM_WURTS_LAYER = 2;
extern int shape(string shape, double x, double y, double z);

int wurtz_layer(int argc, char **argv, optpara opts){
        string atm, atm2, atm3, atm4;
        double conc2 = 0, conc3 = 0, conc4 = 0;
        double lattice_a = 3.0730, lattice_c = 2.5133;//lattice_costant[A]
        double c11, c12, c33, c13, c44, c66;
        int nx = 6, ny = 6, nz = 4;
        int one_layer = 0;
        ...
        if(opts.atomcomp == "zl-3H-Si") {
          atm = "Si";
          lattice_a = 3.84030;
          lattice_c = 9.40676;
          c11 = 0.00591162548254263;
          c12 = -0.000369781552633251;
          c33 = 0.00767769330941004;
          c13 = -0.00213584937950066;
          c44 = 0.0125628140703518;
          c66 = 0.0196270853778214;
        }
        else if(opts.atomcomp == "zl1-3H-Si") {
          atm = "Si";
          one_layer = 1;
          lattice_a = 3.84030;
          lattice_c = 9.40676;
        ...
        if(opts.stress.length() > 0){
                ...
                eyz = c44*syz/2; ezx = c44*szx/2; exy = c66*sxy/2;
	}
		...
		{0.666667, 0.333333, 0}, {0.666667, 0.333333, 0.75}
        };
        Coordinate atom_zl3[2] = {
		{0.333333, 0.666667, 0}, {0.333333, 0.666667, 0.75}
        };
	 ...
	 for(m=0; m<atmlist.size(); m++){
        ...
        int layer;
        m = 0;
        for(k=0; k<=nz; k++){ //takizawa
          for(j=0; j<ny; j++){
            for(i=0; i<nx; i++){
                for(l=0; l<UNIT_ATOM_NUM_WURTS_LAYER; l++){
                      double ax, ay, az;
                      if(k==0 && l==1) {delatm++; continue;}
                      if(k==nz && l==0) {delatm++; continue;}
                      else if(k%3==0) ax = ((atom_zl3[l].x+i) + (atom_zl3[l].y+j)*sin(-30 * M_PI/180))/nx, ay = (atom_zl3[l].y+j)/ny, az = 0.5-(atom_zl3[l].z-k)/(nz*2);
                      else if(k%3==1) ax = ((atom_zl2[l].x+i) + (atom_zl2[l].y+j)*sin(-30 * M_PI/180))/nx, ay = (atom_zl2[l].y+j)/ny, az = 0.5-(atom_zl2[l].z-k)/(nz*2);
                      else if(k%3==2) ax = ((atom_zl1[l].x+i) + (atom_zl1[l].y+j)*sin(-30 * M_PI/180))/nx, ay = (atom_zl1[l].y+j)/ny, az = 0.5-(atom_zl1[l].z-k)/(nz*2);
                      if(ax < -1.0e-07) ax += 1; if(ax > -1.0e-07 && ax < 1.0e-07) ax = 0;
		      layer = nz - k + l;
                      if(k==0 || (k==1 && l==1) ) 
                        cout << atmlist[m] << " " << ax << " " << ay << " " << az << " F" << endl;
                      else
                        cout << atmlist[m] << " " << ax << " " << ay << " " << az << " G " << layer << endl;
                      m++;
                }
              if(!one_layer){
                for(l=0; l<UNIT_ATOM_NUM_WURTS_LAYER; l++){
                      double ax, ay, az;
                      if(k==0 || (k==1 && l==0)) {delatm++; continue;}
                      if(k==nz && l==1) {delatm++; continue;}
                      else if(k%3==0) ax = ((atom_zl2[l].x+i) + (atom_zl2[l].y+j)*sin(-30 * M_PI/180))/nx, ay = (atom_zl2[l].y+j)/ny, az = 0.5-(atom_zl2[l].z+k-1)/(nz*2);
                      else if(k%3==1) ax = ((atom_zl3[l].x+i) + (atom_zl3[l].y+j)*sin(-30 * M_PI/180))/nx, ay = (atom_zl3[l].y+j)/ny, az = 0.5-(atom_zl3[l].z+k-1)/(nz*2);
                      else if(k%3==2) ax = ((atom_zl1[l].x+i) + (atom_zl1[l].y+j)*sin(-30 * M_PI/180))/nx, ay = (atom_zl1[l].y+j)/ny, az = 0.5-(atom_zl1[l].z+k-1)/(nz*2);
                      if(ax < -1.0e-07) ax += 1; if(ax > -1.0e-07 && ax < 1.0e-07) ax = 0;
		      layer = nz - k - l + 1;
                      cout << atmlist[m] << " " << ax << " " << ay << " " << az << " G " << layer << endl;
                      m++;
