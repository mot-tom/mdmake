...
#include "opt.h"
using namespace std;
int UNIT_ATOM_NUM_TETRA_LAYER = 1;

extern int shape(string shape, double x, double y, double z);

int tetra_layer(int argc, char **argv, optpara opts){
        ...
        else if(opts.atomcomp == "zl1-3T-Si") {
          atm = "Si";
          one_layer = 1;
          lattice_a = 3.84030;
          lattice_c = 5.431;
        ...
        Coordinate atom_zl1[1] = {
                {0, 0, 0}
        };
        Coordinate atom_zl2[1] = {
                {0, 0.5, 0}
        };
        Coordinate atom_zl3[1] = {
                {0.5, 0.5, 0}
        };
        Coordinate atom_zl4[1] = {
                {0.5, 0, 0}
        };
	 ...
	 for(m=0; m<atmlist.size(); m++){
        ...
        for(k=0; k<nz; k++){
          for(j=0; j<ny; j++){
            for(i=0; i<nx; i++){
                for(l=0; l<UNIT_ATOM_NUM_TETRA_LAYER; l++){
                      double ax, ay, az;
