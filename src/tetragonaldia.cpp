...
#include "opt.h"
using namespace std;
int UNIT_ATOM_NUM_TETRADIA = 4;

extern int shape(string shape, double x, double y, double z);
 
int tetragonaldia(int argc, char **argv, optpara opts){
        string atm, atm2, atm3, atm4;
        double conc2 = 0, conc3 = 0, conc4 = 0;
        double lattice_a = 3.0730, lattice_c = 5.431;//lattice_costant[A]
        ...
        else if(opts.atomcomp == "3T-SiGe-alloy") {
          atm = "Si";
          atm2 = "Ge";
	   ...
	   reconc2 = (double)atmnum2/atmall;
          ...
          lattice_c = 5.431 + 0.2*reconc2 + 0.027*reconc2*reconc2;
          lattice_a = lattice_c * sqrt(2) / 2.0;
        ...
        else if(opts.atomcomp == "3T-GeSiSn-alloy") {
          atm = "Ge";
          atm2 = "Si";
          atm3 = "Sn";
          if(opts.atomratio.length() > 0) {
	   ...
	   reconc3 = (double)atmnum3/atmall;
          ...
          cerr << "rand     " << opts.seed << endl;
          lattice_c = 5.658 - 0.227*reconc2 + 0.831*reconc3;
          lattice_a = lattice_c * sqrt(2) / 2.0;
        }
        ...
        Coordinate atom[4] = {
                {0, 0, 0}, {0, 0.5, 0.25}, {0.5, 0.5, 0.5}, {0.5, 0, 0.75}
        };
	 ...
	 m = 0;
        for(i=0; i<nx; i++){
          for(j=0; j<ny; j++){
            for(k=0; k<nz; k++){
                for(l=0; l<UNIT_ATOM_NUM_TETRADIA; l++){
