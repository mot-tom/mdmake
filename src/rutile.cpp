...
#include "opt.h"
using namespace std;
int UNIT_ATOM_NUM_RUTILE_Si = 2;
int UNIT_ATOM_NUM_RUTILE_O = 4;
...
int rutile(int argc, char **argv, optpara opts){
        ...
        double lattice_a = 4.177, lattice_c = 2.666;//lattice_costant[A]
        double c11, c12, c44;
        int nx = 4, ny = 4, nz = 4;
        ...
        if(opts.atomcomp == "st-SiO2") {
          atm = "Si";
          lattice_a = 4.177;
          lattice_c = 2.666;
        }
        else if(opts.atomcomp == "st-GeO2") {
          atm = "Ge";
          lattice_a = 4.398;
          lattice_c = 2.863;
        }
        else if(opts.atomcomp == "ru-SnO2") {
          atm = "Sn";
          lattice_a = 4.738;
          lattice_c = 3.187;
        }
        else if(opts.atomcomp == "ru-TiO2") {
          atm = "Ti";
          lattice_a = 4.594;
          lattice_c = 2.959;
        }
        ...
        Coordinate atom_si[2] = {
                {0, 0, 0}, {0.5, 0.5, 0.5}
        };
        Coordinate atom_o[16] = {
                {0.306, 0.306, 0}, {0.694, 0.694, 0}, {0.194, 0.806, 0.5}, {0.806, 0.194, 0.5}
        };
	 ...
	 m = 0;
        for(i=0; i<nx; i++){
          for(j=0; j<ny; j++){
            for(k=0; k<nz; k++){
                for(l=0; l<UNIT_ATOM_NUM_RUTILE_Si; l++){
                      double ax, ay, az;
                      ax = (atom_si[l].x+i)/nx, ay = (atom_si[l].y+j)/ny, az = (atom_si[l].z+k)/nz;
        ...
        for(i=0; i<nx; i++){
          for(j=0; j<ny; j++){
            for(k=0; k<nz; k++){
                for(l=0; l<UNIT_ATOM_NUM_RUTILE_O; l++){
                      double ax, ay, az;
